...
;(async function main() {
  ...
  await i18next.use(i18nextChainedBackend).init({
    ...
    backend: {
      ...
      backendOptions: [{ prefix: 'i18next_' }, { loadPath: './i18n/{{lng}}@0.5.0.0001.json' }]
    },
    defaultNs: 'main',
    ns: ['main'],
    load: 'languageOnly'
  ...
  async function doDiscovery() {
    ...
    const cooldowns = JSON.parse(localStorage.getItem('cooldowns')) || {}
    cooldowns[guid] = {
      t: response.burnout < SERIES ? Date.now() + COOLDOWN * 1000 : response.burnout,
      c: response.burnout < SERIES ? response.burnout : 0
    }
    localStorage.setItem('cooldowns', JSON.stringify(cooldowns))
  ...
  async function showInfo(data) {
    ...
    if (typeof data === 'string') { // дан guid
      request_controllers.points.abort()
  ...
  async function openProfile(data) {
    ...
    Object.keys(response.badges).forEach(title => {
      ...
      next.style.setProperty('--prg', progress)
