;(async function main() {
  ...
  map.on('click', e => {
    ...
    if (piv.length) {
      ...
      const visible = points_source.getFeatures().map(m => ({
        g: m.getId(),
        p: ol.proj.toLonLat(m.getGeometry().getCoordinates())
      }))
      near_points.splice(0, Infinity, ...visible.filter(f => isInRange(f.p)))
    }
  })
  ...
  const navi_state = JSON.parse(localStorage.getItem('navi')) ?? {
    active: false,
    closed: false,
    target: [],
    point: {},
    ...
    if (content.getAttribute('data-tab') == 3) {
      ...
      if (order !== null) {
        ...
        if ((order.t ?? 0) + 5 * 60e3 <= Date.now()) {
          ...
          order.a = response
          localStorage.setItem('refs-arrangement', JSON.stringify({ a: response, t: Date.now() }))
        }
  ...
  document.querySelector('[data-setting="ptcmir"]').addEventListener('change', function() {
    const value = this.checked
    const _ = document.querySelector('.i-buttons')
    if (value) _.classList.add('mirrored')
    else _.classList.remove('mirrored')
    changeSettings('ptcmir', value)
  })
    ...
    response.list.forEach(entry => {
      ...
      $('.notifs__list').append(
        ...
        .append($('<span>', { class: 'notifs__entry-stamp' })
          .append($('<span>', { class: 'notifs__entry-time', text: date.toLocaleString(LANG, { hour: '2-digit', minute: '2-digit', second: '2-digit' }) }))
  ...
  document.querySelector('.navi-sizer').addEventListener('click', function() {
    ...
    localStorage.setItem('navi', JSON.stringify(navi_state))
  })
  ...
  function createInventoryItem(data) {
    ...
    if (data.t == 3) {
      ...
      title.prepend($('<span>', { class: 'level-pill', text: '?' }).css('background', '#000'))
  ...
  function getRefsData(target) {
    ...
    function makeEntry(e, data) {
      ...
      repair.onclick = async function() {
        ...
        let eng = 0
        let eng_total = 0
        response.data.forEach(e => (eng += e.e, eng_total += Cores[e.l]?.eng))
        const percent = eng / eng_total * 100
        e.style.setProperty('--energy', percent + '%')
        e.querySelector('.iid-energy').textContent = `${formatter.format(percent)}%\xa0@\xa0${response.data.length}`
  ...
  async function openProfile(data) {
    ...
    const is_hidden = Object.values(response.stats).every(e => e === null)
    ...
    if (!is_hidden) {
      ...
      msg.className = 'pr-stats__hidden'
      msg.textContent = i18next.t('profile.stats-hidden')
      document.querySelector('.pr-stats').append(msg)
    }
