...
const TEAM_COLORS_CONSOLE = ['⚪','🔴','🟢','🔵']
const TEAM_COLORS = ['#ccc','#f00','#0f0','#00f']
...
async function computeDiff (saved, fresh) {
  ...
  for (let i = 0; i < diff.length; i++) {
    ...
    const diffMessageConsole = point.te === 0 
      ...
      : `${point.t}${t('actionsCapturedMessage')}${TEAM_COLORS_CONSOLE[point.te]}${point.o}`

    const diffMessage = point.te === 0 
      ? `${point.t}<br/>${t('actionsNeutralizedMessage')}`
      : `<span style="color: ${TEAM_COLORS[point.te]}">${point.o}</span><br/>${point.t}`

    log.push({timestamp, ...point})
    console.log(diffMessageConsole)
    if (i < notificationsLimit) { 
      showToast(diffMessage, 'top left') // TODO: сделать настройку?
    }
  }
...
async function GetInview(lat, lon) {
  ...
  const radius = 2500
  const zoom = 15
  const initialPoint = ol.proj.fromLonLat([lon,lat])
  const [w,s] = ol.proj.toLonLat(ol.proj.transform([initialPoint[0]-radius,initialPoint[1]-radius],proj,proj))
  const [e,n] = ol.proj.toLonLat(ol.proj.transform([initialPoint[0]+radius,initialPoint[1]+radius],proj,proj))
